...
def test_get_asset_price(client, asset):
    ...
    # test 404
    url = url_for("api.asset_price_by_ticker", ticker_symbol="unknown", target_currency="unknown")
    resp = client.get(url)
    assert resp.status_code == 404
    
    # test 200
    url = url_for("api.asset_price_by_ticker", ticker_symbol=asset.ticker_symbol, target_currency=asset.currency)
    resp = client.get(url)
    assert resp.status_code == 200
